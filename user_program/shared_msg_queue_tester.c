...
#define MOUSEFILE "/dev/input/event3"                   /*Path to read to get the mouse events*/
#define MAX 1000                                        /*max no. of interval entries tocalculate mean and standard deviation*/
#define CPU_CLOCKS_PER_MILLISECOND 1700000              /*Frequency of CPU in milliseconds*/
...
struct queue *q[2];                                     /*To hold references of two queues*/
...
void* periodic_sender(void *arg)                        /*Thread function for periodic sending threads*/
{
    ...
    {   if(rx_count>MAX)                                /*exit if maximum receive has been acheived*/
        ...
        ret1=sq_write(m,q[(*src)%2]);                       /*message is written to their pertaining queue*/
        if(ret1<0)
            {
                errno=EINVAL;
                free(m);                            /*if queue is full,drop the message and free the memory*/
            }
    else
    printf("msg enqueued by periodic thread with source_id: %d msg_id: %d random_num : %lf @ time %lf ms\n",m->src_id,m->msg_id,m->rand,(double)m->enq_t/CPU_CLOCKS_PER_MILLISECOND);
 ...
 void* aperiodic_sender(void *arg)              /*Thread function for aperiodic sending threads*/
{
    ...
    {   
        ...
        printf("msg enqueued by aperiodic thread with source_id: %d msg_id: %d random_num : %lf @ time %lfms\n",m->src_id,m->msg_id,m->rand,(double)m->enq_t/CPU_CLOCKS_PER_MILLISECOND);  
    }
...
void* periodic_receiver(void *arg)                              /*Thread function for periodic receiving thread*/
{
    ...
    struct msg *m;
    int ret3,i=0;
    while(!(double_click)||!((q[0]->front==q[0]->rear) && !(q[0]->full))||!((q[1]->front==q[1]->rear) && !(q[1]->full))) /*run until double click or queue 1 is empty or queue 2 is empty so that all messages are dequeued even after double click*/
    {
            ret3=sq_read(&m,q[i++%2]); /*read from both the queues alternatively*/
            if(ret3<0)
  ...
  int main(void)
{
   ...
   /*references of queues are set to operate on */ 
    q[0]=sq_create();
    q[1]=sq_create();
